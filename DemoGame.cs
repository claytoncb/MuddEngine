...
namespace MuddEngine
{
    class DemoGame : MuddEngine.MuddEngine
    {
        ...
        {
            player = new Player(new Vector3(0,0,10),"PlayerHead",0, 150f,"Sprite");
            ...
            AddLight(light1);
            ceilingLight1 = new Sprite2D(player.Position + new Vector3(0,0, 18f), "CeilingLight1",1, 0f,"Sprite");

            light2 = new LightSource(player.Position + new Vector3(512,0,17f), 800f, 1.0f, Raylib_cs.Color.Red, "RedLight");
            AddLight(light2);
            ceilingLight2 = new Sprite2D(player.Position + new Vector3(512,0, 18f), "CeilingLight2",1, 0f,"Sprite");

            light3 = new LightSource(player.Position + new Vector3(-512,0,17f), 800f, 1.0f, Raylib_cs.Color.Green, "GreenLight");
            AddLight(light3);
            ceilingLight3 = new Sprite2D(player.Position + new Vector3(-512,0, 18f), "CeilingLight3",1, 0f,"Sprite");

            light4 = new LightSource(player.Position + new Vector3(-1024,0,17f), 800f, 1.0f, Raylib_cs.Color.White, "WhiteLight");
            AddLight(light4);
            ceilingLight4 = new Sprite2D(player.Position + new Vector3(-1024,0, 18f), "CeilingLight4",1, 0f,"Sprite");
            for (int j = 0; j < 3; j++)
            {
                for (int i = 0; i < 20; i++)
                {
                    Sprite2D Tile = new Sprite2D(new Vector3(-1536 + 128*i,128 - 128*j,0),$"Tile{i}-{j}",0,0,"Tile", false);
                }
            }
        }
