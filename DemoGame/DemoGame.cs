...
namespace MuddEngine.DemoGame
{
    class DemoGame : MuddEngine.MuddEngine
    {
        ...
        public override void OnLoad()
        {
            ...
            for (int i = 0; i < 17; i++)
                {
                    _ = new Sprite($"FloorTop{i}", new Vector3(-256 + 32 * i, 32, 0), 0)
                    {
                    ...
                    _ = new Sprite($"FloorLower{i}", new Vector3(-256 + 32 * i, -64, 0), 0)
                    {
                        SheetLocation = new (0, 576),
                        VisibleSize = new(32, 16),
                        VisibleOffset = new(0,8),
                    ...
                    _ = new Sprite($"Carpet{i}", new Vector3(-256 + 32 * i, -16, 0), 0)
                    {
                        SheetLocation = new (i==0?64:(i==16?32:0), 608),
                        VisibleSize = new(32, 32),
                        isFlat = true
                    };
                    _ = new Sprite($"WallBottom{i}", new Vector3(-256 + 32 * i, 80, 0), 0)
                    {
                        SheetLocation = new (32*(i%4==0?1:(i%4==1 || i%4==3)?2:0)*2, 640),
                        VisibleSize = new(32, 32),
                    };
                    _ = new Sprite($"WallTop{i}", new Vector3(-256 + 32 * i, 80, 32), 0)
                    {
